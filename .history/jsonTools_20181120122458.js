...
function randomizeData(jsonObj, iter){
	var randomizedRecords = [];
	for (var record in jsonObj){
		...
		randomizedRecords.push(JSON.parse(JSON.stringify(changedRecords)));
		fs.writeFileSync('Mock_' + i + singleRecord.First_Name + singleRecord.Last_Name + '.json', 
						JSON.stringify(changedRecords, null, 1), 'utf8', function (err) {
			if (err) {
