...
function randomizeData(jsonObj){
	...
	for (var record in jsonObj){
		...
		for (var i = 1; i < 25; i++){
			...
			console.log('changedRecords so far: \n' + JSON.stringify(changedRecords, null, 1));
		}
		var content = JSON.stringify(changedRecords, null, 1);
		console.log('content before saving to file: \n' + content);
		fs.writeFileSync('Mock_' + i-1 + singleRecord.First_Name + singleRecord.Last_Name + '.json', content, 'utf8', function (err) {
