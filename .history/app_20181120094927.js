...
async function uploadRecords(req, res){
	...
	if (req.params.record.endsWith('.csv')){
		var csvjson = jsonTools.csvJSON(req_body);
		jsonObj = jsonTools.validateJson(JSON.parse(csvjson));
	} else {
		jsonObj = jsonTools.validateJson(req_body);
	}
	var fileName = req.params.record + '.json';
	console.log('Validated json file: ' + JSON.stringify(jsonObj, null, 1));
	lines = jsonTools.formatForDataflow(jsonObj, "\n");
	...
	jsonTools.randomizeData(jsonObj);
