...
function validateJson(jsonObj){
	for (var i in jsonObj){
		...
		var date = (jsonObj[i].hasOwnProperty('Date\r')) ? jsonObj[i]['Date\r'] : ( jsonObj[i]['Date'] );			
		if (date.includes('/') != -1){
...
function randomizeData(jsonObj, iter){
	...
	for (var record in jsonObj){
		...
		// var fileName = 'Mock_' + i + singleRecord.First_Name + singleRecord.Last_Name + '.json';
	...
	var fileName = 'Mock_all_' + iter + '.json';
