...
function randomizeData(jsonObj){
	...
	for (var record in jsonObj){
		...
		for (var i = 0; i < 3; i++){
			...
			console.log(i + ': pushed ' + JSON.stringify(changedRecords[i], null, 1));
		}
		console.log('before writing to file: \n' + JSON.stringify(changedRecords, null, 1));
		fs.writeFileSync('Mock_' + i-1 + singleRecord.First_Name + singleRecord.Last_Name + '.json', 
