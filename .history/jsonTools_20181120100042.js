...
function validateJson(jsonObj){
	for (var i in jsonObj){
		console.log('starting validate json: ' + JSON.stringify(jsonObj, null, 1));
...
function csvJSON(csv){
	console.log('csvJSON raw csv ' + csv);
  var lines=csv.split("\n");
  var result = [];
...
function randomizeData(jsonObj){
	...
	for (var record in jsonObj){
		...
		fs.writeFileSync('Mock_' + singleRecord.First_Name + singleRecord.Last_Name + '.json', JSON.stringify(changedRecords, null, 1), 'utf8', function (err) {
